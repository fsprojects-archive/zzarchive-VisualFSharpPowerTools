...
type LanguageService (?backgroundCompilation: bool, ?projectCacheSize: int, ?fileSystem: IFileSystem) =
    ...
    let parseAndCheckFileInProject(filePath, source, options) = async { 
        ...
        let results = 
            ...
            | Choice1Of2 (parseResults, FSharpCheckFileAnswer.Succeeded checkResults) ->
                ...
                debug "[LanguageService] ParseAndCheckFileInProject - HasFullTypeCheckInfo? %b" 
                    checkResults.HasFullTypeCheckInfo
                debug "[LanguageService] ParseAndCheckFileInProject - Errors? %A" 
                    checkResults.Errors
    ...
    member __.OnFileClosed filePath = 
        ...
        | true, _ -> 
            debug "[LanguageService] %s was removed from `files` dictionary" filePath
        | _ -> ()
    ...
    member x.GetCheckerOptions(fileName, projFilename, source, files, args, referencedProjects, fscVersion) =
        ...
        let opts = 
            ...
            else // We are in a project - construct options using current properties
                async { return x.GetProjectCheckerOptions(projFilename, files, args, referencedProjects) }
        opts
    ...
    member __.GetScriptCheckerOptions(fileName, projFilename, source, fscVersion) = async {
        ...
        try 
            ...
            let! opts = 
                checkerInstance.GetProjectOptionsFromScript
                    (fileName, source, fakeDateTimeRepresentingTimeLoaded projFilename)
                        ...
                        |> Option.map (fun path -> 
                            ...
                            { opts with 
                                ...
                                [|  yield fsharpCoreRef
                                    ...
                                    |> Seq.filter (fun s -> not (s.Contains "FSharp.Core.dll")) 
                                |] 
                            })
            ...
            debug "GetScriptCheckerOptions: ProjectFileName: %s, ProjectFileNames: %A, FSharpProjectOptions: %A,\
                IsIncompleteTypeCheckEnvironment: %A, UseScriptResolutionRules: %A" 
                results.ProjectFileName results.ProjectFileNames results.OtherOptions 
                results.IsIncompleteTypeCheckEnvironment results.UseScriptResolutionRules
            return results
        with e -> 
    ...
    member __.GetProjectCheckerOptions(projFilename, files, args, referencedProjects) =
        ...
        debug "GetProjectCheckerOptions: ProjectFileName: %s, ProjectFileNames: %A, FSharpProjectOptions: %A, IsIncompleteTypeCheckEnvironment: %A, UseScriptResolutionRules: %A, ReferencedProjects: %A" 
            opts.ProjectFileName opts.ProjectFileNames opts.OtherOptions 
            opts.IsIncompleteTypeCheckEnvironment opts.UseScriptResolutionRules opts.ReferencedProjects
    ...
    member internal __.TryGetStaleTypedParseResult (fileName:string, options, src, stale)  = 
        ...
        let res = 
            ...
            | AllowStaleResults.MatchingFileName -> 
                checkerInstance.TryGetRecentCheckResultsForFile (fileName, options) 
            | AllowStaleResults.MatchingSource -> 
                checkerInstance.TryGetRecentCheckResultsForFile (fileName, options, source=src)
    ...
    member x.GetAllUsesOfAllSymbolsInFile 
        (projectOptions, fileName, source: string, stale, checkForUnusedOpens) : SymbolUse[] Async = async {
        ...
        let allSymbolsUses =
            fsharpSymbolsUses |> Array.map (fun symbolUse -> 
            let fullNames = 
    ...
    member x.GetAllEntitiesInProjectAndReferencedAssemblies 
        (projectOptions: FSharpProjectOptions, fileName, source, ?withCache) = async {
    ...
    member x.GetIdentTooltip 
        (line, colAtEndOfNames, lineStr, names, project: FSharpProjectOptions, file, source) = async {
