...
open FSharpVSPowerTools.Logging
...
type NavigableItemCache () =
    ...
    let loadFromDisk (solutionPath: FilePath) =
        protect <| fun _ ->
            ...
            pf.Stop()
            logfn  "[NavigableItemCache] Loaded: %d items for %d files. Sutable: %d items for %d files %s" 
        ...
        if !dirty then
            ...
            protect <| fun _ ->
                ...
                let items = pf.Time "Save to file" <| fun _ ->
                    ...
                    items
                pf.Stop()
                logfn "[NavigableItemCache] Saved %d items for %d files to %s %s" 
                            (items |> Array.sumBy (fun x -> x.Items.Length)) items.Length filePath pf.Result

    let tryGetSolutionPath() = Option.attempt (fun () -> "placeholder")  
    ...
    member __.TryGet (file: FileDescriptor): NavigableItem[] option =
        ...
        | true, x when x.Descriptor.LastWriteTime = file.LastWriteTime -> 
            logfn "[NavigableItemCache] Found for %s, %O" file.Path file.LastWriteTime
            Some x.Items
        | true, _x ->
            ...
            None
        | _ -> 
            logfn "[NavigableItemCache] Not found for %s" file.Path
            None
